...
void startGame() {
  ...
  // New 22 Point Game logic
    int maxScore = 0; // Tracks the highest score among players
  int winningPlayer = 0; // Tracks the player with the highest score
  int eliminatedPlayer = 0; // Tracks the player who gets eliminated

  // Loop through each player
  for (int i = 0; i < numberOfPlayers; i++) {
    currentPlayer = i + 1;
    int playerScore = 0; // Initialize player score for this turn
    ...
    display.setCursor(0, 30);
    display.print("Score: ");
    display.print(playerScores[currentPlayer - 1]); // Display current score
    display.display();
    ...
    while (true) {
      ...
      if (digitalRead(BUTTON_MINUS_1) == LOW) {
        ...
        display.setCursor(0, 30);
        display.print("Score: ");
        display.print(playerScores[currentPlayer - 1]);
        display.display();
      }
      if (digitalRead(BUTTON_PLUS_1) == LOW) {
        delay(200); // debounce
      ...
      if (digitalRead(BUTTON_PLUS_3) == LOW) {
        delay(200); // debounce
        playerScores[currentPlayer - 1] += 3;
        display.clearDisplay();
        display.setTextSize(2);
        display.setTextColor(SSD1306_WHITE);
        display.setCursor(0, 10);
      ...
      if (digitalRead(BUTTON_NEXT) == LOW) {
        delay(200); // debounce
        while (digitalRead(BUTTON_NEXT) == LOW); // Wait until button is released
        break; // Exit loop to move to the next player
      }
    }

    // Check for player elimination
    if (playerScores[currentPlayer - 1] <= -10) {
      eliminatedPlayer = currentPlayer;
      playerScores[currentPlayer - 1] = -100; // Mark player as eliminated
    }

    // Check for player reaching 22 points or more
    if (playerScores[currentPlayer - 1] >= 22) {
      if (playerScores[currentPlayer - 1] > maxScore) {
        maxScore = playerScores[currentPlayer - 1];
        winningPlayer = currentPlayer;
      }
    }
  }
  ...
  if (winners == 1) {
    ...
    // Find player with highest score among winners
    int highestScore = 0;
    for (int i = 0; i < numberOfPlayers; i++) {
      if (playerScores[i] >= 22 && playerScores[i] != -100 && playerScores[i] > highestScore) {
    ...
    delay(2000); // Let the user read the message
    gameOver = true;
  } else if (eliminatedPlayer != 0) {
    // Display player eliminated message
    display.clearDisplay();
    display.setTextSize(1);
    display.setTextColor(SSD1306_WHITE);
    display.setCursor(0, 10);
    display.print("PLAYER ");
    display.print(eliminatedPlayer);
    display.println(" ELIMINATED!");
    display.display();
    delay(2000); // Let the user read the message
  }

  if (!gameOver) {
    // Continue to next round if game is not over
    delay(2000); // Let the user read the message
    startGame();
  }
  }
